...
inline void storeWB(double *a, const double *b, double w, int len) {
    const size_t nBlockWidth = 16;
    const size_t cntRem      = len & (nBlockWidth - 1);
    const size_t cntBlock    = len / nBlockWidth;
    size_t i;
    __m256d avxSum0, avxSum1;
    __m256d avxSum2, avxSum3;
    __m256d avxLoadB0, avxLoadB1;
    __m256d avxLoadB2, avxLoadB3;
    const __m256d avxWeight0 = _mm256_set1_pd(w);
    const __m256d avxWeight1 = _mm256_set1_pd(w);
    const __m256d avxWeight2 = _mm256_set1_pd(w);
    const __m256d avxWeight3 = _mm256_set1_pd(w);
    ...
    for (i = 0; i < cntBlock; ++i) {
        ...
        avxSum0  = _mm256_mul_pd(avxLoadB0, avxWeight0);
        avxSum1  = _mm256_mul_pd(avxLoadB1, avxWeight1);
        avxSum2  = _mm256_mul_pd(avxLoadB2, avxWeight2);
        avxSum3  = _mm256_mul_pd(avxLoadB3, avxWeight3);
        if (alignA) {
            _mm256_store_pd(a,      avxSum0);
            _mm256_store_pd(a + 4,  avxSum1);
            _mm256_store_pd(a + 8,  avxSum2);
            _mm256_store_pd(a + 12, avxSum3);
        }
        else {
            _mm256_storeu_pd(a,      avxSum0);
            _mm256_storeu_pd(a + 4,  avxSum1);
            _mm256_storeu_pd(a + 8,  avxSum2);
            _mm256_storeu_pd(a + 12, avxSum3);
        }
        a += nBlockWidth;
        b += nBlockWidth;
    }
